...
class AdminController extends AbstractController
{
    ...
    public function evaluations(EvaluationPeriodRepository $repo, DepartmentRepository $deptRepo, AcademicYearRepository $ayRepo, UserRepository $userRepo, SubjectRepository $subjectRepo, EvaluationResponseRepository $responseRepo, SuperiorEvaluationRepository $superiorEvalRepo): Response
    {
        ...
        // Build distinct list of colleges from departments
        $departments = $deptRepo->findAllOrdered();
        $colleges = [];
        foreach ($departments as $d) {
            $cn = $d->getCollegeName();
        ...
        // Merge evaluator counts from both SET/SEF responses and Superior evaluations
        ...
        // Build a map of faculty full name -> employment status (position)
        $facultyPositionMap = [];
    ...
    #[Route('/evaluations/{id}/toggle', name: 'admin_evaluation_toggle', methods: ['POST'])]
    ...
    #[Route('/evaluations/{id}/edit', name: 'admin_evaluation_edit', methods: ['POST'])]
    public function editEvaluation(EvaluationPeriod $eval, Request $request, EntityManagerInterface $em): Response
    {
        if ($this->isCsrfTokenValid('edit_eval' . $eval->getId(), $request->request->get('_token'))) {
            ...
            foreach ($targets as $target) {
                ...
                // Keep per-item identity fields intact when editing merged SET rows.
    ...
    #[Route('/evaluations/{id}/lock-results', name: 'admin_evaluation_lock_results', methods: ['POST'])]
    ...
    #[Route('/evaluations/{id}/delete', name: 'admin_evaluation_delete', methods: ['POST'])]
    public function deleteEvaluation(EvaluationPeriod $eval, Request $request, EntityManagerInterface $em): Response
    {
        if ($this->isCsrfTokenValid('delete_eval' . $eval->getId(), $request->request->get('_token'))) {
            // Remove related superior evaluations
            $em->createQuery('DELETE FROM App\Entity\SuperiorEvaluation se WHERE se.evaluationPeriod = :ep')
                ->setParameter('ep', $eval)
                ->execute();
            // Remove related evaluation messages
